...
def main() -> int:
    ...
    assert result["comparison_name"] == "resnet18_pytorch_cuda_fp32_vs_onnxruntime_cpu_fp32_vs_onnxruntime_cuda_fp32_vs_onnxruntime_tensorrt_fp32_vs_tensorrt_fp16"
    assert [runtime["name"] for runtime in result["runtimes"]] == ["pytorch_cuda", "onnxruntime_cpu", "onnxruntime_cuda", "onnxruntime_tensorrt", "tensorrt_trtexec"]
    ...
    assert result["ratios"]["mean_latency_onnxruntime_cuda_over_tensorrt"] > 1.0
    assert result["ratios"]["mean_latency_onnxruntime_tensorrt_over_tensorrt"] > 1.0
    assert "ONNX Runtime CPU" in markdown
    assert "ONNX Runtime CUDA" in markdown
    assert "ONNX Runtime TensorRT" in markdown
