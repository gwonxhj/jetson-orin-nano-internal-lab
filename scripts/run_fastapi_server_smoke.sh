...
PORT="${FASTAPI_PORT:-18080}"
BASE_URL="http://${HOST}:${PORT}"
RESULT_REL="results/inference/fastapi_resnet18_server_${STAMP}.json"
REPORT_REL="docs/reports/fastapi_resnet18_server_smoke.md"
SERVER_LOG_REL="artifacts/system/fastapi_resnet18_server_${STAMP}.log"
TEGRSTATS_REL="artifacts/system/tegrastats_fastapi_resnet18_${STAMP}.log"
...
if command -v tegrastats >/dev/null 2>&1; then
  ...
  printf 'tegrastats unavailable; FastAPI smoke will run without thermal/power side log\n' >&2
  TEGRSTATS_REL=""
fi
...
python3 "${ROOT_DIR}/benchmarks/inference/fastapi_resnet18_client_smoke.py" \
  --base-url "${BASE_URL}" \
  --output "${RESULT_FILE}" \
  --report "${REPORT_FILE}" \
