...
| Multi-workload interaction | Closed as 30-second probe, 10-minute sustained run, 30-minute V1 gap run, compact runtime timeline export, p99/burst-window report, and bounded degradation signal | [Multi-workload sustained runtime](multi_workload_sustained_runtime.md), [Multi-workload runtime timeline](multi_workload_runtime_timeline.md), [Multi-workload burst window report](multi_workload_burst_window_report.md), [Multi-workload degradation signal](multi_workload_degradation_signal.md), [Multi-workload InferEdge export](multi_workload_sustained_inferedge_export.md) |
...
| 4 | Runtime degradation signal | At least one bounded overload or contention scenario records latency spike, resource pressure, queue buildup, dropped request, or fallback behavior. | Closed by `multi_workload_degradation_signal_20260518_013625`: opt-in concurrency 8 scenario, FastAPI p99 delta +578.9529ms, RAM avg delta +84.282MB, no request errors. | Proceed to queue/serving observability. |
| 5 | Queue/serving observability | Serving evidence includes request counts, error counts, queue/backlog proxy, and dropped/failed request count. | `/metrics` and request counts exist; queue depth/backlog is not explicit. | Add lightweight in-process counters or client-side outstanding-request timeline. |
| 6 | InferEdge consumer handoff | Multi-workload artifacts have a documented handoff path for InferEdge Runtime/Orchestrator/AIGuard/Lab. | `metadata.json` / `result.json` exist; ecosystem consumer mapping is not fully documented. | Add a short mapping report that explains which fields each InferEdge component should consume. |
| 7 | Schema drift protection | CI validates every committed `metadata.json` / `result.json`, including multi-workload fields and artifact hashes. | Current schema validation passes for 11 handoff pairs. | Keep validation green; add tests when timeline, degradation, or new handoff roles are introduced. |
...
5. Next: add queue/serving observability and decide whether degradation signal needs an InferEdge handoff role.
6. Document InferEdge ecosystem consumer mapping.
7. Cut a V1 release only after schema validation and public-safety checks pass.
...
V1 is ready when the repo contains a reproducible 30-minute multi-workload runtime scenario, timeline-aware telemetry evidence, at least one bounded degradation signal, InferEdge-compatible handoff artifacts, schema validation, public-safety review, and a release narrative that clearly states what the evidence proves and does not prove.
