...
이 프로젝트의 evidence는 **환경 조건 -> runtime 비교 -> serving layer -> audio inference -> InferEdge-compatible handoff** 순서로 읽으면 가장 자연스럽습니다. 각 결과는 재현 조건과 한계를 함께 기록하며, 짧은 smoke benchmark만으로 deployment-ready를 주장하지 않습니다.
...
| 4 | Serving layer | ResNet18 inference를 localhost FastAPI API로 감싸 client/server latency와 API 사용 흐름을 확인합니다. | [FastAPI server smoke](fastapi_resnet18_server_smoke.md), [API usage](fastapi_api_usage.md), [Serving boundary](serving_boundary_notes.md) |
...
| FastAPI serving | Can local inference be exposed through a reproducible localhost API smoke path? | [FastAPI API usage](fastapi_api_usage.md) | `results/inferedge/resnet18_fastapi_serving_20260514_142053/result.json` |
...
- A localhost FastAPI serving layer can produce structured result evidence without claiming production readiness.
