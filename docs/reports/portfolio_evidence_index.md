...
| LLM text readiness | Can local text-generation plumbing be introduced without mutating the stable benchmark env first? | [LLM env candidate probe](llm_env_candidate_probe.md), [LLM text generation smoke](llm_text_generation_smoke.md) | `llm_env` tiny-gpt2 CUDA path smoke succeeded; stable `yolo_env` remains unmodified |
